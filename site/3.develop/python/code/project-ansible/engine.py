...
class ExeutionEngine:
  ...
  def format_output(self, Host, task: Task):
    """格式化输出"""
    status = "ok" if task.status == "success" else "failed"
    header = f"Task [{task.name}] {'*' * 50}"
    result = f"{status}: [{Host.address}]\n"
  ...
  def run(self):
    ...
    with ThreadPoolExecutor() as executor:
      ...
      for future in futures:
        ...
        print(self.format_output(host, task))
